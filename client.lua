			...
			print(PlayerData.job.name)
				if PlayerData.job.name == 'ambulance' and PlayerData.job.onduty then 
					--	SpawnVehicle(GetEntityCoords(PlayerPedId()))
						...
						Notify("支払い完了"..Config.Price, "success Please wait")
						Wait(1000)
						Notify("治療完了までお待ちください")
						Wait(10000)
						TriggerEvent("hospital:client:Revive")
						StopScreenEffect('DeathFailOut')
					--	Notify("Medic is arriving")				
						Notify("治療完了")
						TriggerEvent('wasabi_crutch:giveCrutch', 2400)
				elseif EMSOnline <= Config.Doctor and hasEnoughMoney and spam then
					--	SpawnVehicle(GetEntityCoords(PlayerPedId()))
					TriggerServerEvent('hhfw:charge')
					Notify("支払い完了"..Config.Price, "success Please wait")
					Wait(1000)
